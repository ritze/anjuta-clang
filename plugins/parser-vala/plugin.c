...
#define PREFS_BUILDER PACKAGE_DATA_DIR"/glade/anjuta-parser-vala.ui"
#define ICON_FILE "anjuta-language-cpp-java-plugin.png"

/* Preferences keys */
#define ANJUTA_PREF_SCHEMA_PREFIX "org.gnome.anjuta."
#define PREF_SCHEMA "org.gnome.anjuta.plugins.parser-vala"

static gpointer parent_class;

/* Enable/Disable language-support */
static void
install_support (ParserValaPlugin *parser_plugin)
{
...
static void
uninstall_support (ParserValaPlugin *parser_plugin)
{
    if (!parser_plugin->support_installed)
        return;
...
{
    ParserValaPlugin *parser_plugin;
    IAnjutaDocument* doc = IANJUTA_DOCUMENT(g_value_get_object (value));
    parser_plugin = ANJUTA_PLUGIN_PARSER_VALA (plugin);
...
static gboolean
parser_vala_plugin_activate_plugin (AnjutaPlugin *plugin)
{
    ParserValaPlugin *parser_plugin;

    parser_plugin = ANJUTA_PLUGIN_PARSER_VALA (plugin);

    DEBUG_PRINT ("%s", "AnjutaParserValaPlugin: Activating plugin ...");
...
static gboolean
parser_vala_plugin_deactivate_plugin (AnjutaPlugin *plugin)
{
    ParserValaPlugin *parser_plugin;
    parser_plugin = ANJUTA_PLUGIN_PARSER_VALA (plugin);
    ...
    DEBUG_PRINT ("%s", "AnjutaParserValaPlugin: Deactivated plugin.");
    return TRUE;
}

static void
parser_vala_plugin_finalize (GObject *obj)
{
...
static void
parser_vala_plugin_dispose (GObject *obj)
{
    ParserValaPlugin* plugin = ANJUTA_PLUGIN_PARSER_VALA (obj);
    /* Disposition codes */
...
static void
parser_vala_plugin_instance_init (GObject *obj)
{
    ParserValaPlugin *plugin = ANJUTA_PLUGIN_PARSER_VALA (obj);
...
static void
parser_vala_plugin_class_init (GObjectClass *klass)
{
    AnjutaPluginClass *plugin_class = ANJUTA_PLUGIN_CLASS (klass);

    parent_class = g_type_class_peek_parent (klass);

    plugin_class->activate = parser_vala_plugin_activate_plugin;
    plugin_class->deactivate = parser_vala_plugin_deactivate_plugin;
    klass->finalize = parser_vala_plugin_finalize;
    klass->dispose = parser_vala_plugin_dispose;
}
...
{
    GError* error = NULL;
    ParserValaPlugin* plugin = ANJUTA_PLUGIN_PARSER_VALA (ipref);
...
ANJUTA_PLUGIN_BEGIN (ParserValaPlugin, parser_vala_plugin);
ANJUTA_PLUGIN_ADD_INTERFACE (ipreferences, IANJUTA_TYPE_PREFERENCES);
ANJUTA_PLUGIN_END;

ANJUTA_SIMPLE_PLUGIN (ParserValaPlugin, parser_vala_plugin);

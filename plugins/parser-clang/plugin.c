...
#define PREFS_BUILDER PACKAGE_DATA_DIR"/glade/anjuta-parser-clang.ui"
#define ICON_FILE "anjuta-parser-clang-plugin.png"
...
static void
install_support (ParserClangPlugin *parser_plugin)
{
...
static void
uninstall_support (ParserClangPlugin *parser_plugin)
{
    if (!parser_plugin->support_installed)
        return;
...
static gboolean
parser_clang_plugin_deactivate_plugin (AnjutaPlugin *plugin)
{
    ParserClangPlugin *parser_plugin;
    parser_plugin = ANJUTA_PLUGIN_PARSER_CLANG (plugin);
...
static void
parser_clang_plugin_finalize (GObject *obj)
{
...
static void
parser_clang_plugin_dispose (GObject *obj)
{
...
static void
parser_clang_plugin_instance_init (GObject *obj)
{
    ParserClangPlugin *plugin = ANJUTA_PLUGIN_PARSER_CLANG (obj);
...
static void
parser_clang_plugin_class_init (GObjectClass *klass)
{
    AnjutaPluginClass *plugin_class = ANJUTA_PLUGIN_CLASS (klass);

    parent_class = g_type_class_peek_parent (klass);

    plugin_class->activate = parser_clang_plugin_activate_plugin;
    plugin_class->deactivate = parser_clang_plugin_deactivate_plugin;
    klass->finalize = parser_clang_plugin_finalize;
    klass->dispose = parser_clang_plugin_dispose;
}
...
{
    GError* error = NULL;
    ParserClangPlugin* plugin = ANJUTA_PLUGIN_PARSER_CLANG (ipref);
...
ANJUTA_PLUGIN_BEGIN (ParserClangPlugin, parser_clang_plugin);
ANJUTA_PLUGIN_ADD_INTERFACE (ipreferences, IANJUTA_TYPE_PREFERENCES);
ANJUTA_PLUGIN_END;

ANJUTA_SIMPLE_PLUGIN (ParserClangPlugin, parser_clang_plugin);
